...
🔍 ``` index=auth action=success | table _time user src ```
...
🔍 ``` index=auth action=success | stats count by user ```
...
🔍 ```index=auth action=success| stats count by src```
...
🔍``` index=auth action=success | stats latest(_time) as last_login by user```
...
🔍 ```index=auth action=success | timechart span=1h count ```
